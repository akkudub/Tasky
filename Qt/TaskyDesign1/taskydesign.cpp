...
TaskyDesign::TaskyDesign(QWidget *parent): QMainWindow(parent){
	wordlist<<"add ''"<<"from"<<"to"<<"delete"<<"display";
	ui.setupUi(this);
	...
	connect(ui.InputBox, SIGNAL(textChanged(const QString& text)), this, SLOT(updateStatusBar()));
...
void TaskyDesign::updateStatusBar(){
	QString input=ui.InputBox->text();
